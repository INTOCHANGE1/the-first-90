...
export function PerfectMorningClient({ initial }: { initial: Shape }) {
  ...
  const { data, setData, status, lastSavedAt, flush } = useAutosave<Shape>(
    ...
    async (next) => {
      ...
      const { error } = await supabase.from("morning_night_routines").upsert(
        { user_id: user.id, routine_type: "morning", ...next },
  ...
  return (
    <div className="flex flex-col gap-8">
      <section className="flex flex-col gap-3">
        <MicroLabel>{PERFECT_MORNING.descriptionLabel}</MicroLabel>
        <p className="text-sm text-ash">{PERFECT_MORNING.descriptionHint}</p>
        <Textarea
      ...
      <section className="flex flex-col gap-3">
        <MicroLabel>{PERFECT_MORNING.nonNegotiablesLabel}</MicroLabel>
      ...
      <section className="flex flex-col gap-3">
        <MicroLabel>{PERFECT_MORNING.reflectionLabel}</MicroLabel>
        <Textarea
      ...
      <section className="flex flex-col gap-3">
        <MicroLabel>{PERFECT_MORNING.benefitsLabel}</MicroLabel>
        <Textarea
          value={data.benefits ?? ""}
          onChange={(e) =>
            setData((p) => ({ ...p, benefits: e.target.value }))
          }
          onBlur={() => void flush()}
          bloodAccent
        />
      ...
      <FormFooter
        ...
        flush={flush}
        nextHref="/journal/phase/1/perfect-night"
        canContinue={filled}
      />
    </div>
  );
